...
frappe.listview_settings['Leave Policy Assignment'] = {
	onload: function (list_view) {
		...
		list_view.page.add_inner_button(__("Auto Leave Policy Assignment"), function () {
			me.dialog = new frappe.ui.form.MultiSelectDialog({
				doctype: "Employee",
				target: cur_list,
				setters: {
				...
				data_fields: [
				{
					fieldname: 'assignment_based_on',
					fieldtype: 'Select',
					options: ["", "Leave Period" , "Date of Rejoining"],
					label: __('Assignment Based On'),
					onchange: () => {
						if (cur_dialog.fields_dict.assignment_based_on.value === "Leave Period") {
							cur_dialog.set_df_property("effective_from", "read_only", 1);
						...
						else if (cur_dialog.fields_dict.assignment_based_on.value === "Date of Rejoining") {
							cur_dialog.set_df_property("effective_from", "read_only", 1);
							cur_dialog.set_df_property("effective_to", "read_only", 1);
							cur_dialog.set_df_property("effective_from", "reqd", 0);
							cur_dialog.set_df_property("effective_to", "reqd", 0);
							cur_dialog.set_df_property("leave_period", "reqd", 0);
						} else {
							cur_dialog.set_df_property("effective_from", "read_only", 0);
							cur_dialog.set_df_property("leave_period", "reqd", 0);
				...
				action(employees, data) {
					frappe.call({
						method: 'attendance.attendance.doctype.leave_policy_assignment.leave_policy_assignment.auto_create_assignment_for_multiple_employees',
						async: false,
						args: {
							employees: employees,
							data: data
