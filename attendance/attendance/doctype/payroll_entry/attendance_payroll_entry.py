...
class PayrollEntry(Document):
	...
	def get_salary_components_detailed(self, component_type):
		salary_slips = self.get_sal_slip_list(ss_status=1, as_dict=True)
		if salary_slips:
			salary_components = frappe.db.sql(
				"""
	...
	def get_salary_component_total_detailed(self, component_type=None):
		salary_components = self.get_salary_components_detailed(component_type)
		if salary_components:
			component_dict = {}
			for item in salary_components:
				add_component_to_accrual_jv_entry = True
				...
				if add_component_to_accrual_jv_entry:
					key = (item.salary_component,
							item.payroll_cost_center, item.employee)
					component_dict[key] = component_dict.get(
						key, 0) + flt(item.amount)
			account_details = self.get_account_detailed(
	...
	def get_account_detailed(self, component_dict=None):
		account_dict = {}
		for key, amount in component_dict.items():
			account = self.get_salary_component_account(key[0])
			key_d = (account, key[1], key[2])
			account_dict[key_d] = account_dict.get(key_d, 0) + amount
		return account_dict

	def make_accrual_jv_entry(self):
		merge_entries = frappe.db.get_single_value(
			"Payroll Settings", 'merge_accrual_journal_entry_in_payroll_entry') or 0
		if merge_entries:
			return self.make_merged_accrual_jv_entry()
		else:
			return self.make_detailed_accrual_jv_entry()

	def make_detailed_accrual_jv_entry(self):
		self.check_permission("write")
		earnings = self.get_salary_component_total_detailed(
			component_type="earnings") or {}
		deductions = self.get_salary_component_total_detailed(
			...
			for acc_cc, amount in deductions.items():
				...
				accounts.append(
					self.update_accounting_dimensions(
						{
							"account": payroll_payable_account,
							"debit_in_account_currency": flt(payable_amt, precision),
							"exchange_rate": flt(exchange_rate),
							"cost_center": self.cost_center,
							"reference_type": self.doctype,
							"reference_name": self.name,
	...
	def make_merged_accrual_jv_entry(self):
		self.check_permission("write")
		...
		if salary_slip_name_list and len(salary_slip_name_list) > 0:
			...
			for salary_slip_name in salary_slip_name_list:
				...
				for sal_detail in salary_slip.earnings:
					...
					if only_tax_impact != 1 and statistical_component != 1:
						if is_flexible_benefit == 1 and creat_separate_je == 1:
							...
							employee_total += sal_detail.amount
				for sal_detail in salary_slip.deductions:
					...
					if statistical_component != 1:
						salary_slip_total -= sal_detail.amount
						employee_total -= sal_detail.amount
				employees_total.append(frappe._dict(
					{"employee": salary_slip.employee, 'amount': employee_total}))
			if salary_slip_total > 0:
				...
				if merge_entries:
					...
					self.create_journal_entry_detailed(
						salary_slip_total,employees_total, "salary")
	...
	def create_journal_entry_detailed(self, je_payment_amount, sal_slip_list, user_remark):
		payroll_payable_account = self.payroll_payable_account
		precision = frappe.get_precision(
			"Journal Entry Account", "debit_in_account_currency")
		...
		for salary in sal_slip_list :
			exchange_rate, amount = self.get_amount_and_exchange_rate_for_journal_entry(
				payroll_payable_account, salary.amount or 0, company_currency, currencies
			)
			accounts.append(
				self.update_accounting_dimensions(
					{
