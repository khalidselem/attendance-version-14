...
frappe.ui.form.on("Payroll Entry", {
  ...
  refresh: function (frm) {
    if (frm.doc.docstatus == 0) {
      if (!frm.is_new()) {
        ...
        frm
          .add_custom_button(__("Get Employees"), function () {
    ...
    frappe.call({
      ...
      callback: function (r) {
        if (r.message && !r.message.submitted) {
          frm
            .add_custom_button("Make Bank Entry", function () {
              make_bank_entry(frm);
            })
            .addClass("btn-primary");
        }
  ...
  setup: function (frm) {
    ...
    frm.set_query("employee", "employees", () => {
      ...
      let message = __("Mandatory fields required in {0}", [
        __(frm.doc.doctype),
      ]);
  ...
  get_employee_filters: function (frm) {
    ...
    if (frm.doc.employees) {
      let employees = frm.doc.employees
        .filter((d) => d.employee)
        .map((d) => d.employee);
