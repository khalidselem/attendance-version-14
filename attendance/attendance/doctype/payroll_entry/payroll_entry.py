...
class PayrollEntry(Document):
        ...
        if employees:
            ...
            if len(employees) > 30:
                frappe.enqueue(create_salary_slips_for_employees,
                                timeout=600, employees=employees, args=args)
            else:
    ...
    def submit_salary_slips(self):
        ...
        if len(ss_list) > 30:
            frappe.enqueue(submit_salary_slips_for_employees, timeout=600,
                            payroll_entry=self, salary_slips=ss_list)
        else:
    ...
    def get_salary_component_account(self, salary_component):
        account = frappe.db.get_value("Salary Component Account",
            {"parent": salary_component, "company": self.company}, ["account","payable_account"],as_dict=True)

        if not account or not account.account:
            frappe.throw(_("Please set account in Salary Component {0}")
                .format(salary_component))

        if not account or not account.payable_account:
            frappe.throw(_("Please set Payable account in Salary Component {0}")
                .format(salary_component))
        ...
        if salary_components:
            ...
            for item in salary_components:
                ...
                if add_component_to_accrual_jv_entry:
                    ...
                    if item.customer :
                        cost_center = frappe.db.get_value("Customer",item.customer,"cost_center") or item.payroll_cost_center
                    # row_key = (item.salary_component,cost_center, item.customer , item.employee)
                    row_key = (item.salary_component,cost_center, item.customer)
                    component_dict[row_key] \
                        = component_dict.get(row_key, 0) + flt(item.amount)
            account_details = self.get_account(component_dict = component_dict)
            # frappe.throw(str(account_details))
    ...
    def get_account(self, component_dict = None):
        ...
        for key, amount in component_dict.items():
            ...
            # row_key = (account ,key)
            # account_dict[row_key] = account_dict.get(row_key, 0) + amount
            item = frappe._dict()
            item.amount = amount
            # item.employee = key[3]
            item.customer =  key[2]
            item.cost_center =  key[1]
            item.salary_component =  key[0]
            item.account = account.account
            item.payable_account = account.payable_account
            account_dict.append(item)
        return account_dict

    def make_accrual_jv_entry(self):
        # frappe.msgprint("write")
        self.check_permission("write")
        ...
        credit_precision = frappe.get_precision("Journal Entry Account", "credit_in_account_currency")
        ...
        if earnings or deductions:
            ...
            # Earnings
            for row in earnings:
                exchange_rate, amt = self.get_amount_and_exchange_rate_for_journal_entry(row.account, row.amount, company_currency, currencies)
                exchange_rate2, amt2 = self.get_amount_and_exchange_rate_for_journal_entry(row.payable_account, row.amount, company_currency, currencies)
                ...
                accounts.append(self.update_accounting_dimensions({
                    ...
                    # "user_remark":frappe.db.get_value("Employee",row.employee,"employee_name")  + " - " + row.salary_component
					
                }, accounting_dimensions))
                accounts.append(self.update_accounting_dimensions({
                    "account": row.payable_account,
                    "credit_in_account_currency": flt(amt2, credit_precision),
                    "exchange_rate": flt(exchange_rate2),
                    "project": self.project,
                    "cost_center": row.cost_center or self.cost_center,
                    "customer": row.customer,
                    # "party_type": 'Employee',
            ...
            for row in deductions:
                ...
                accounts.append(self.update_accounting_dimensions({
                    "account": row.account,
                    "credit_in_account_currency": flt(amt, credit_precision),
                    "exchange_rate": flt(exchange_rate),
                    "project": self.project,
                    "cost_center": row.cost_center or self.cost_center,
                    "customer": row.customer,
                ...
                accounts.append(self.update_accounting_dimensions({
                    "account": row.payable_account,
                    "debit_in_account_currency": flt(amt2, precision),
                    "exchange_rate": flt(exchange_rate2),
                    "project": self.project,
                    "cost_center": row.cost_center or self.cost_center,
                    "customer": row.customer,
	...
	for emp in employees:
		if emp not in salary_slips_exists_for:
			args.update({
				"doctype": "Salary Slip",
				"employee": emp,
                "project" : frappe.db.get_value("Employee" , emp , 'project')
			})
...
def submit_salary_slips_for_employees(payroll_entry, salary_slips, publish_progress=True):
    ...
    for ss in salary_slips:
        ss_obj = frappe.get_doc("Salary Slip",ss[0])
        ss_obj.project = frappe.db.get_value("Employee" , ss_obj.employee , 'project')
        if ss_obj.net_pay<0:
